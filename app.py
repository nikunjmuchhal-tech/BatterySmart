...
st.markdown(
    "<style>"
    "[data-testid='stAppViewContainer'] { background: linear-gradient(160deg, #eafff6 0%, #ffffff 35%, #eef0ff 100%); }"
    "[data-testid='stHeader'] { background: rgba(0,0,0,0); }"
    "h1 { font-size: 2.2rem !important; color:" + BRAND_NAVY + " !important; font-weight: 800 !important; }"
    ".stTabs [data-baseweb='tab'] { font-size: 1.1rem !important; padding: 8px 16px !important; }"
    ".stTabs [data-baseweb='tab-list'] { gap: 6px; }"
    ".stTabs [aria-selected='true'] { color: " + BRAND_GREEN + " !important; }"
    "div[data-testid='stMarkdownContainer'] p { font-size: 1.02rem; }"
    ".detail-name { font-size: 1.5rem; font-weight: 700; margin-bottom: 4px; color:" + BRAND_NAVY + "; }"
    ".detail-label { font-size: 0.8rem; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; }"
    ".detail-value { font-size: 1.05rem; margin-bottom: 8px; }"
    ".metric-box { background: linear-gradient(135deg, " + BRAND_NAVY + " 0%, #2a2496 100%); border-radius:12px; padding:14px 8px; text-align:center; }"
    ".metric-num { font-size:1.7rem; font-weight:800; color: white; }"
    ".metric-label { font-size:0.75rem; color:" + BRAND_GREEN + "; text-transform:uppercase; letter-spacing:0.4px; }"
...
def render_detail(item, sheet, df, unique_key):
    ...
    if item["stage"] == "D+1":
        ...
        dfe_value = st.selectbox("Did the DFE/dealer ask the driver to pay any fees? (optional)", DFE_OPTIONS, index=dfe_default_idx, key=dfe_key)
    ...
    if st.button("Save", key=save_key, type="primary"):
        updates = {}
...
def render_dashboard_stage(items):
    ...
    if all_rows:
        ...
        st.dataframe(table_df, use_container_width=True, hide_index=True, height=min(400, 45 + 35 * len(table_df)))
    else:
